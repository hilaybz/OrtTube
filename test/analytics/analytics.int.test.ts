...
describe.skipIf(!online)("analytics (compute-on-read)", () => {
  ...
  let singleQ: AuthoredQuestion;
  let multiQ: AuthoredQuestion;
  ...
  beforeAll(async () => {
    ...
    await perfectAttempt.answerCorrectly(singleQ);
    await perfectAttempt.answerCorrectly(multiQ);
  ...
  describe("quiz_stats", () => {
    it("counts attempts/completions and averages score (anonymized attempts count)", async () => {
      ...
      expect(stats.attempt_count).toBe(4);
      expect(stats.completion_count).toBe(3);
  ...
  describe("question_stats", () => {
    it("reports correct% and a distractor distribution including soft-deleted options", async () => {
      ...
      expect(single.total_answers).toBe(3);
      expect(single.correct_count).toBe(1);
      ...
      expect(multi.correct_count).toBe(2);
  ...
  describe("class_stats", () => {
    it("reports attempt-based completion (anonymized counts) and roster-based coverage", async () => {
      ...
      expect(stats.current_member_count).toBe(3);
      ...
      expect(classQuiz.completion_count).toBe(3);
  ...
  describe("tutor_stats", () => {
    it("counts tutor volume and flags likely answer-extraction attempts (quiz scope)", async () => {
      ...
      expect(stats.anonymized_count).toBe(1);
